...
public:
	...
	inline std::vector<TilesResult> fromPolygon(int zoom, Polygon polygon, int detailLevel = 1);
...
// FROM POLYGON
inline std::vector<TilesResult> Tiles::fromPolygon(int zoom, Polygon polygon, int detailLevel)
{
	return this->fromPolygon(zoom, polygon.getCoordinates(), detailLevel);
};
...
inline std::vector<TilesResult> Tiles::fromPolygon(int zoom, std::vector<Coordinate> polygonPoints, int detailLevel)
{
	...
	for(int lev = 1; lev <= detailLevel; lev++)
	{
		...
		{
			Polygon tmp(box);
			std::vector<BoundingBox> tmptmp = tmp.getCoveringRectangles(p, true);
